// components/DeviceMapComponent.jsx
...
export const getDonorColor = (donorName) => {
  if (!donorName) return "#2ecc71";
...
// Custom marker icon with blinking/pulsing animation
export const createCustomIconHtml = (color, isSelected = false, isBlinking = true) => {
...
// Popup HTML template
export const createPopupHtml = (device, color) => {
...
const DeviceMapComponent = ({ 
    ...
    markersRef.current = filteredDevices.map(item => {
      ...
      try {
        ...
        const customIcon = L.divIcon({
          ...
          html: createCustomIconHtml(color, isSelected, true),
        ...
        marker.on('click', () => {
          ...
          if (onDeviceSelect) onDeviceSelect(item);
  ...
  return (
    <MapWrapper customHeight={customHeight}>
      {showHeader && (
        <div className="map-header">
          <div className="d-flex justify-content-between align-items-center">
            <div>
              <h4 className="mb-0">🌍 Device Location Map</h4>
              <small>Real-time device tracking</small>
            </div>
            ...
            {showFilters && (
              <div className="d-flex gap-3">
                ...
                <div className="stat-badge" style={{ background: '#10b981' }} title="Devices with valid coordinates">
        ...
        <div ref={mapRef} className="map-wrapper">
          ...
          {mapLoaded && validDevices.length === 0 && (
            <div className="no-data-overlay">
              <NoData message={`No devices with valid coordinates found.`} />
            </div>
          )}
        </div>
      </div>
